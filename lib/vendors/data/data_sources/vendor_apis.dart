...
class VendorsApis {
  ...
  static Future<GetProductsByVendorResponse?> getAllProductsFromVendor(
      {
      required  int vendorId,
        int pageNumber = 1,
        int pageSize = 10
      }) async {
    String endPoint = ApiUrls.GET_PRODUCTS_BY_VENDOR_ENDPOINT;
    String? accessToken =
        MyApp.navKey.currentState?.context.read<AuthCubit>().accessToken;
    ...
    try {
      ...
      print(response.data);
      return GetProductsByVendorResponse.fromJson(response.data);
    } catch (e) {
      if (kDebugMode) {
        print(e);
      }
