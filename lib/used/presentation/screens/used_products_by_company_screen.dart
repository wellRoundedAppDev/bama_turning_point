...
import 'package:classic_eccomerce/used/presentation/cubit/used_products_cubit.dart';
import 'package:classic_eccomerce/used/presentation/screens/used_product_details_screen.dart';
...
class UsedProductsByCompanyScreen extends StatelessWidget {
  const UsedProductsByCompanyScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        body: BlocProvider(
          create: (context) => UsedProductsCubit(byClients: false)..init(),
          child: BlocConsumer<UsedProductsCubit, UsedProductsStates>(
              listener: (context, state) {},
              builder: (context, state) {
                UsedProductsCubit cubit = UsedProductsCubit.get(context);
                List<UsedProduct>? usedProducts = cubit.products;

                return (state is UsedProductsLoadingState)
                    ? const Center(
                        child: CircularProgressIndicator(),
                      )
                    : (state is UsedProductsNetworkConnectionFailedState)
                        ? Padding(
                            padding: const EdgeInsets.all(16.0),
                            child: NoNetworkRefreshPage(
                              refresh: () async {
                                await cubit.setProductsByCompany();
                              },
                            ),
                          )
                        : RefreshIndicator(
                            ...
                            child: ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  var usedProductItem = usedProducts?[index];
                                  return GestureDetector(
                                    onTap: () {
                                      Navigator.push(
                                          context,
                                          PageTransition(
                                              child: UsedProductDetailsScreen(
                                                  usedProductItem:
                                                      usedProductItem),
                                              type: PageTransitionType
                                                  .leftToRight));
                                    },
                                    child: UsedProductItem(
                                      usedProductItem: usedProductItem,
                                    ),
                                  );
                                },
                                ...
                                itemCount: usedProducts?.length ?? 0),
                          );
